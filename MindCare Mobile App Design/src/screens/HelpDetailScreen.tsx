...
const HelpDetailScreen = ({ resource, onBack }: HelpDetailScreenProps) => {
    ...
    return (
        <Screen className="bg-card">
            <TopBar showBack onBack={onBack} title={t("help-detail.title")} />
            <div className="px-6 py-6 pb-24">
                ...
                <div className="flex flex-wrap justify-center gap-2 mb-8">
                    {resource.badges.map((badge: string, i: number) => (
                        <span
                            key={i}
                            className="px-3 py-1 rounded-full bg-mc-peach/10 text-mc-peach text-[12px] font-bold uppercase tracking-wider border border-mc-peach/20"
                        >
                            {badge}
                        </span>
                    ))}
                ...
                <div className="space-y-6">
                    <section>
                        <h3 className="font-bold text-foreground mb-2 flex items-center gap-2">
                            <Info size={16} /> {t("help-detail.section.about")}
                        </h3>
                    ...
                    <section>
                        <h3 className="font-bold text-foreground mb-2 flex items-center gap-2">
                            <Clock size={16} /> {t("help-detail.section.hours")}
                        </h3>
                    ...
                    <div className="bg-secondary/20 p-5 rounded-[16px] border border-secondary/30 mt-4">
                        <h3 className="font-bold text-foreground mb-2 text-[15px]">
                            {t("help-detail.section.when-to-contact")}
                        </h3>
            ...
            <div className="absolute bottom-0 left-0 right-0 p-4 bg-card border-t border-border flex gap-3 z-50 shadow-[0_-4px_20px_rgba(0,0,0,0.1)]">
                ...
                {resource.phone && (
                    <McButton
                        ...
                        icon={Phone}
                    >
                        {t("help.button.call")} {resource.phone}
                    </McButton>
                )}
            </div>
        </Screen>
