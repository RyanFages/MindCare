  ...
  return (
    <Screen className="bg-card">
      <TopBar
        ...
        onBack={onBack}
        rightAction={
          <button onClick={handleToggleFav} className="p-1" aria-label={fav ? 'Retirer des favoris' : 'Enregistrer'}>
            <Bookmark size={24} className={cn(fav ? 'fill-primary text-primary' : 'text-muted-foreground')} />
          </button>
        }
      />
      <div className="px-6 py-4">
        <div className="mb-6">
          ...
          {content.type === 'Témoignage' && (content as any).author && (
            <div className="mt-4 flex items-center gap-3">
              ...
              <div>
                <p className="font-display font-semibold text-foreground text-[15px]">{(content as any).author}, {(content as any).age} ans</p>
                {(content as any).theme && (
                  <span className="text-[11px] font-medium text-accent bg-accent/10 px-2 py-0.5 rounded-full">{(content as any).theme}</span>
                )}
              </div>
            </div>
          )}
        ...
        {content.type === 'Témoignage' && (
          <div className="mt-6 flex items-center gap-3 p-3 bg-muted/50 rounded-[12px] border border-border">
            ...
            <p className="text-[11px] text-muted-foreground leading-snug">
              Ce témoignage a été partagé avec le consentement de son auteur et modéré avant publication.
            </p>
          </div>
        )}

        <div className="mt-12 mb-8 space-y-4">
          <McButton fullWidth onClick={handleToggleFav} variant={fav ? 'secondary' : 'primary'}>
            <Bookmark size={18} className={cn("mr-2", fav && "fill-current")} />
            {fav ? 'Retirer des favoris' : 'Enregistrer pour plus tard'}
          </McButton>
