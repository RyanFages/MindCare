...
const NotificationsScreen = ({ onBack }: { onBack: () => void }) => {
    ...
    const DAYS = [
        t("notifications.day.monday"),
        t("notifications.day.tuesday"),
        t("notifications.day.wednesday"),
        t("notifications.day.thursday"),
        t("notifications.day.friday"),
        t("notifications.day.saturday"),
        t("notifications.day.sunday"),
    ];
    ...
    const ToggleItem = ({
        ...
        <div className="flex items-center justify-between py-4 border-b border-border last:border-0">
            <div className="pr-4 flex items-start gap-3">
                ...
                <div>
                    <div className="text-foreground font-medium font-body text-[15px]">
                        {label}
                    </div>
                    <div className="text-muted-foreground text-[13px] leading-snug mt-0.5">
                        {desc}
                    </div>
                </div>
            </div>
    ...
    return (
        <Screen className="bg-card">
            <TopBar title={t("notifications.title")} showBack onBack={onBack} />
            <div className="px-6 pt-2 pb-6 flex flex-col h-full overflow-y-auto">
                ...
                <McCard className="px-4 py-2 mb-4 border-primary/20">
                    <div className="flex items-center gap-2 pt-3 pb-1">
                        ...
                        <span className="text-foreground font-display font-semibold text-[15px]">
                            {t("notifications.section.weekly")}
                        </span>
                    </div>
                    ...
                    <ToggleItem
                        label={t("notifications.toggle.enable")}
                        desc={t("notifications.toggle.description")}
                        isOn={settings.weeklyCheckin}
                        onToggle={() =>
                            update({ weeklyCheckin: !settings.weeklyCheckin })
                        }
                    />
                    ...
                    {settings.weeklyCheckin && (
                        <div className="pb-4 pt-2 space-y-4 pl-1">
                            ...
                            <div className="bg-muted/50 border border-border rounded-2xl p-4 mt-2">
                                <p className="text-muted-foreground text-[12px] font-body uppercase tracking-wide mb-1.5">
                ...
                <McCard className="px-4">
                    <ToggleItem
                        icon={Bell}
                        label={t("notifications.daily.title")}
                        desc={t("notifications.daily.desc")}
                        isOn={settings.daily}
                        onToggle={() => update({ daily: !settings.daily })}
                    />
                    <ToggleItem
                        icon={Bell}
                        label={t("notifications.content.title")}
                        desc={t("notifications.content.desc")}
                        isOn={settings.content}
                        onToggle={() => update({ content: !settings.content })}
                    />
                    <ToggleItem
                        icon={Heart}
                        label={t("notifications.support.title")}
                        desc={t("notifications.support.desc")}
                        isOn={settings.support}
                        onToggle={() => update({ support: !settings.support })}
                    />
                </McCard>
