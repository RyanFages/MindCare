...
import { getAllEntries, addEntry, deleteEntry, type JournalEntry } from '@/lib/journalTracker';
...
const JournalScreen = ({ onNavigate, onBack }: JournalScreenProps) => {
  ...
  return (
    <Screen bottomBar={<BottomBar activeTab="home" onNavigate={onNavigate} />}>
      <TopBar title="Mon journal" showBack onBack={onBack} />

      <div className="px-4 space-y-6 pb-4">
        {/* New entry */}
        {isWriting ? (
          <McCard className="p-4 space-y-4">
            <H2 className="text-[18px]">Nouvelle entrée</H2>
            <textarea
              value={text}
            ...
            <div className="flex gap-3">
              <McButton variant="secondary" className="flex-1" onClick={() => { setIsWriting(false); setText(''); }}>
                Annuler
              </McButton>
              <McButton variant="primary" className="flex-1" onClick={handleSave} disabled={!text.trim()}>
                Enregistrer
              </McButton>
            </div>
          </McCard>
          ...
          >
            ...
            <div>
              <h4 className="font-display font-medium text-foreground text-[16px]">Écrire une entrée</h4>
              <p className="font-body text-[13px] text-muted-foreground">Exprime librement tes pensées</p>
            </div>
          </McCard>
        )}
        ...
        {entries.length > 0 ? (
          <div className="space-y-3">
            <H2 className="text-[18px] px-1">Mes entrées</H2>
            {entries.map((entry) => {
              const isExpanded = expandedId === entry.id;
              return (
                <McCard key={entry.id} className="p-4 space-y-2">
                  <button
                  ...
                  >
                    <div className="text-left">
                      ...
                      <p className="font-body text-[12px] text-muted-foreground">{formatTime(entry.date)}</p>
                    </div>
                    <ChevronRight
                      size={18}
          ...
          <div className="text-center py-12 space-y-3">
            ...
            <TextBody className="text-muted-foreground">Ton journal est vide pour l'instant.</TextBody>
            <TextBody className="text-muted-foreground text-[13px]">Commence par écrire ta première entrée ✨</TextBody>
          </div>
        ) : null}
      </div>
    </Screen>
