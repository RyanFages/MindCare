...
const SectionHeader = ({
    ...
    <div className="flex items-start gap-3 mb-4 mt-6">
        ...
        <div>
            <h2 className="text-[18px] font-medium font-display text-foreground leading-tight">
                {title}
            </h2>
...
const ResourceCard = ({
    ...
    onOpenResource: (r: HelpResource) => void;
    t: any;
}) => (
    <McCard className="p-4 mb-3 active:scale-[0.99] transition-transform">
        <div className="flex justify-between items-start mb-2">
            <div>
                <h3 className="font-bold text-[16px] font-display text-foreground">
                    {resource.name}
                </h3>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
            {resource.badges.slice(0, 3).map((badge: string, i: number) => (
                <span
                    key={i}
                    className="text-[10px] font-medium text-muted-foreground bg-background px-2 py-1 rounded-md border border-border"
                >
                    {badge}
                </span>
            ))}
        ...
        >
            <div className="pt-2 px-4 pb-4">
                <div className="mb-6 mt-4">
                    <H1 className="mb-2">{t("help.title")}</H1>
                ...
                {isSupportNeeded && (
                    <div className="bg-destructive/10 border border-destructive/20 p-4 rounded-[16px] flex gap-3 mb-6">
                        ...
                        />
                        <div>
                            <h3 className="font-bold text-destructive text-[15px] mb-1">
                                {t("help.alert.title")}
                            </h3>
                            <p className="text-[13px] text-foreground leading-snug">
                                {t("help.alert.description")}
                            </p>
                        </div>
                    </div>
                )}
                ...
                <div className="space-y-2 pb-4">
                    <SectionHeader
                        icon={Moon}
                        title={t("help.section.immediate")}
                        subtitle={t("help.section.immediate.subtitle")}
                    />
                    ...
                    <SectionHeader
                        icon={HeartHandshake}
                        title={t("help.section.student")}
                        subtitle={t("help.section.student.subtitle")}
                    />
                    ...
                    <div className="mt-8 p-4 rounded-[16px] bg-background border border-border text-center">
                        <p className="text-[12px] text-mc-text-muted mb-2">
                            {t("help.emergency.label")}
                        </p>
                        <div className="flex justify-center gap-4">
                            <div className="flex items-center gap-2">
                                <div className="w-8 h-8 rounded-full bg-destructive text-destructive-foreground flex items-center justify-center font-bold text-xs">
                                    15
                                </div>
                                <span className="font-bold text-foreground">
                                    {t("help.emergency.ambulance")}
                                </span>
                            </div>
                            <div className="flex items-center gap-2">
                                <div className="w-8 h-8 rounded-full bg-destructive text-destructive-foreground flex items-center justify-center font-bold text-xs">
