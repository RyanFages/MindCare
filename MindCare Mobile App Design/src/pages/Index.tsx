...
const Index = () => {
  ...
  const [resultType, setResultType] = useState<'balanced' | 'watch' | 'support'>('balanced');
    ...
    <div className="h-[100dvh] w-full bg-background font-body text-foreground mx-auto max-w-md shadow-2xl overflow-hidden relative selection:bg-mc-peach">
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {currentScreen === 'breathing-exercise' && <BreathingExerciseScreen onBack={goBack} />}
          {currentScreen === 'history' && <HistoryScreen onNavigate={navigate} onBack={goBack} />}
          ...
          {currentScreen === 'journal' && <JournalScreen onNavigate={navigate} onBack={goBack} />}
        </motion.div>
      </AnimatePresence>
    </div>
  );
